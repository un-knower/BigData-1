...
import somethingUtils.SerializeUtil.ObjectSerializeUtil;
...
//	}
	 ...
	 * @param map	值
	 */
	synchronized public void updateHash(String key, String field, String value) {
		Jedis jedis = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @param expireTime
	 */
	public  void setObject( String msgkey, Serializable data, int expireTime) {
		Jedis jedis = null;
		try {
			jedis = getJedis4Write();
			String key = msgkey;
			jedis.set(key, ObjectSerializeUtil.getStrFromObj(data));
			if(expireTime!=-1){
	 ...
	 * @return
	 */
	public  Object getObject(String msgkey) {
		Jedis jedis = null;
		try {
			jedis = getJedis4Read();
			String key = msgkey;
			String str = jedis.get(key);
			return ObjectSerializeUtil.getObjFromStr(str);
		} catch (Exception e) {
		   e.printStackTrace();
		} finally {
			if (jedis != null)
