...
// to do so by the SparkStreaming.streaming context.
//
  ...
  with Logging
{
  ...
  // should stop if 'isStopped' is true, as the SparkStreaming.streaming context thinks the
  // receiver has stopped. But that's not enough -- the loop also needs to
  // exit if the SparkStreaming.streaming context has asked it to by calling onStop() above.
  //
...
object CustomStreaming {
  def main (args: Array[String]) {
    ...
    println("*** starting SparkStreaming.streaming")
    ssc.start()
    ...
    new Thread("Streaming Termination Monitor") {
      override def run() {
        try {
          ...
          case e: Exception => {
            println("*** SparkStreaming.streaming exception caught in monitor thread")
            e.printStackTrace()
          }
        }
        println("*** SparkStreaming.streaming terminated")
      }
    }.start()
    ...
    println("*** stopping SparkStreaming.streaming")
    ssc.stop()
