...
// SparkStreaming.streaming context on which it is registered. However, it sums the record
...
object Monitoring {
  def main (args: Array[String]) {
    ...
    // SparkStreaming.streaming context, not the stream, so it monitors all the streams, and
