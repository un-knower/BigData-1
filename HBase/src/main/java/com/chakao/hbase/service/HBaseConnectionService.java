package com.chakao.hbase.service;
...
public interface HBaseConnectionService {
	...
	/**
	 * 根据rowkeys获取返回值,返回值为List
	 * <Object> 传递的参数会需要返回值的类型.同时Object必须包含@ORMHBaseColumn 与@ORMHBaseColumnTable
	 * 
	 * @param <T>
	 * 
	 * @param rowKey
	 ...
	 * 
	 *                                                     其他字段的family,
	 *                                                     qualifier值与HBase库中的一致
	 *                                                     .
	 *                                                     如果库中没有family需要手动创建family
	 *                                                     .qualifier则可以任意插入.
	 * 
	 * @param objs
	 */
	...
	/**
	 * 给予表名称 插入Object... 数据到相关的表中,Object中声明的
	 * 
	 * @param <T>
	 * @ORMHbaseTable 中定义的HBase表名称无效.
	 *                Object中的Id必须为Rowkey,同时必须定义id的HBaseColumn的两个值为Rowkey.类似于:
	 * @ORMHBaseColumn(family="rowkey",qualifier="rowkey") private String id;
	 * 
	 *                                                     其他字段的family,
	 *                                                     qualifier值与HBase库中的一致
	...
	/**
	 * 根据Object获取到 @ORMHBaseTable
	 * 中的tableName.然后基于rowkeys删除数据.如果不需要定义Object则可以直接调用 delete(String
	 * tableName,String...rowkeys)
	 * 
	 * @param <T>
	 * 
	 * @param rowkey
	...
	/**
	 * 传递表名称与familys列簇创建表. 如果表名已经存在,那么不会创建表.直接打印日志.
	 * 
	 * @param tableName
	 * @param columnFamilys
	 */
