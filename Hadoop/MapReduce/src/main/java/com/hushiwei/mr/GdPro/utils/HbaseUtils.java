...
public class HbaseUtils {
	...
	/**
	 * 
	 * 10.10.112.18:2181,10.10.103.230:2181,10.10.104.4:2181
	 * 构造函数加载配置
	 */
	public HbaseUtils(){
		System.out.println("初始化hbase。。");
		conf = new Configuration();
		conf.set("hbase.zookeeper.quorum", "172.16.100.102:2181");
		conf.set("hbase.rootdir", "hdfs://ttyc102:9000/hbase");
		try {
...
//		//获取表的所有数据
//		hbase.getALLData("users");
//		//删除一条记录
//		hbase.deleteOneRecord("users","xiaoming");
//		//删除表
...
//		hbase.getScanData("stu","cf","age");
	 ...
	 * 单条插入（一条记录，多个列）
	 */
	public void addOneRecord(String rowkey,String cf,String tableName,String c1,String d1,
			String c2,String d2,String c3,String d3){
		HTable hTable = null;
		...
		p1.add(Bytes.toBytes(cf),Bytes.toBytes(c1),Bytes.toBytes(d1))
		.add(Bytes.toBytes(cf),Bytes.toBytes(c2),Bytes.toBytes(d2))
		.add(Bytes.toBytes(cf),Bytes.toBytes(c3),Bytes.toBytes(d3));
		try {
			hTable.put(p1);
		} catch (Exception e) {
			e.printStackTrace();
